...
class PegasusTracker():
    def __init__(self,local_storage_dir="",wf_dir="",wf=None,rc=None,tc=None,sc=None,execution_site_name="condorpool",dagfile="workflow.yml",config_file=DEFAULT_CONFIG_DIR+"pegasus_data_config.conf") -> None:
        ...
        if self.config.has_section('git'):
            ...
            self.branch=self.config.get("git","pull")
    ...
    def build_metadata(self):
        ...
        if self.use_git:
            if self.pull: 
                metadata_output_combiner_job.add_args(f"-files {' '.join([x.lfn for x in self.metadata_version_outputs])} -output_file_name {self.metadata_file} -file_type yaml --use-git -pull -token {self.token} -owner {self.owner} -repo {self.repo} -branch {self.branch}")
            else:
