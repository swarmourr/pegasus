...
cat <<EOF > local_clustering_local_clustering_worker-3_novalocal-cont.sh
#!/bin/sh
...
export HOME
EOF
container_env /srv >> local_clustering_local_clustering_worker-3_novalocal-cont.sh
cat <<EOF2 >> local_clustering_local_clustering_worker-3_novalocal-cont.sh
...
pegasus-kickstart  -n local_clustering -N local_clustering_worker-3.novalocal -R condorpool  -L federated-learning-example-tracker -T 2024-02-20T14:12:44+00:00 ./local_clustering -f worker-3.novalocal_data.csv -dist ref_dist.npy -node worker-3.novalocal
set -e
printf "\n################[Container] Exiting pegasus lite container script ################\n"  1>&2
EOF2


chmod +x local_clustering_local_clustering_worker-3_novalocal-cont.sh
