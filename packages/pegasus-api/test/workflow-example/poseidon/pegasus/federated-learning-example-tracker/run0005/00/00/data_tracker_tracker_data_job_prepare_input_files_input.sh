...
cat <<EOF > data_tracker_tracker_data_job_prepare_input_files_input-cont.sh
#!/bin/sh
...
export HOME
EOF
container_env /srv >> data_tracker_tracker_data_job_prepare_input_files_input-cont.sh
cat <<EOF2 >> data_tracker_tracker_data_job_prepare_input_files_input-cont.sh
...
pegasus-kickstart  -n data_tracker -N tracker_data_job_prepare_input_files_input -R condorpool  -s pegasus-data/metadata_prepare_input_files_input.yaml=pegasus-data/metadata_prepare_input_files_input.yaml -L federated-learning-example-tracker -T 2024-02-20T19:12:32+00:00 ./data_tracker -files source_data -data_dir pegasus-data -metadata_format yaml -o pegasus-data/metadata_prepare_input_files_input -file_type inputs -pfn /home/poseidon/workflows/FL-workflow/federated-learning-fedstack-PM/data/oneyeardata.csv -gdrive -remote_id 1KLUNrJNfCDI8Y5iXBmASUbtwqCpaHg3L -gcredentials cred
set -e
printf "\n################[Container] Exiting pegasus lite container script ################\n"  1>&2
EOF2


chmod +x data_tracker_tracker_data_job_prepare_input_files_input-cont.sh
