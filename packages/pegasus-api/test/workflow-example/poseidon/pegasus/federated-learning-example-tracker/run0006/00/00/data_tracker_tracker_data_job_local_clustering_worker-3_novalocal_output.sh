...
cat <<EOF > data_tracker_tracker_data_job_local_clustering_worker-3_novalocal_output-cont.sh
#!/bin/sh
...
export HOME
EOF
container_env /srv >> data_tracker_tracker_data_job_local_clustering_worker-3_novalocal_output-cont.sh
cat <<EOF2 >> data_tracker_tracker_data_job_local_clustering_worker-3_novalocal_output-cont.sh
...
pegasus-kickstart  -n data_tracker -N tracker_data_job_local_clustering_worker-3.novalocal_output -R condorpool  -s pegasus-data/metadata_local_clustering_worker-3.novalocal_output.yaml=pegasus-data/metadata_local_clustering_worker-3.novalocal_output.yaml -L federated-learning-example-tracker -T 2024-02-20T19:25:35+00:00 ./data_tracker -files worker-3.novalocal_clusters.csv -data_dir pegasus-data -metadata_format yaml -o pegasus-data/metadata_local_clustering_worker-3.novalocal_output -file_type outputs -pfn /home/poseidon/pegasus-integration/pegasus/packages/pegasus-api/test/workflow-example/worker-3.novalocal_clusters.csv -gdrive -remote_id 1KLUNrJNfCDI8Y5iXBmASUbtwqCpaHg3L -gcredentials cred
set -e
printf "\n################[Container] Exiting pegasus lite container script ################\n"  1>&2
EOF2


chmod +x data_tracker_tracker_data_job_local_clustering_worker-3_novalocal_output-cont.sh
