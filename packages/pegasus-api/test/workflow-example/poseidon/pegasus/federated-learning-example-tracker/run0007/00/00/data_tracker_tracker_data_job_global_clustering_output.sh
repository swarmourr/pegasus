...
cat <<EOF > data_tracker_tracker_data_job_global_clustering_output-cont.sh
#!/bin/sh
...
export HOME
EOF
container_env /srv >> data_tracker_tracker_data_job_global_clustering_output-cont.sh
cat <<EOF2 >> data_tracker_tracker_data_job_global_clustering_output-cont.sh
...
pegasus-kickstart  -n data_tracker -N tracker_data_job_global_clustering_output -R condorpool  -s pegasus-data/metadata_global_clustering_output.yaml=pegasus-data/metadata_global_clustering_output.yaml -L federated-learning-example-tracker -T 2024-02-20T19:25:35+00:00 ./data_tracker -files global_clustering_results.csv -data_dir pegasus-data -metadata_format yaml -o pegasus-data/metadata_global_clustering_output -file_type outputs -pfn /home/poseidon/pegasus-integration/pegasus/packages/pegasus-api/test/workflow-example/global_clustering_results.csv -gdrive -remote_id 1KLUNrJNfCDI8Y5iXBmASUbtwqCpaHg3L -gcredentials cred
set -e
printf "\n################[Container] Exiting pegasus lite container script ################\n"  1>&2
EOF2


chmod +x data_tracker_tracker_data_job_global_clustering_output-cont.sh
