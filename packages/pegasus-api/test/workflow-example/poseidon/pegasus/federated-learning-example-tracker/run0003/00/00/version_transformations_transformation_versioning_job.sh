...
cat <<EOF > version_transformations_transformation_versioning_job-cont.sh
#!/bin/sh
...
export HOME
EOF
container_env /srv >> version_transformations_transformation_versioning_job-cont.sh
cat <<EOF2 >> version_transformations_transformation_versioning_job-cont.sh
...
/bin/chmod +x version_transformations

set +e
job_ec=0
printf "\n#########################[Container] Launching user task #########################\n"  1>&2

pegasus-kickstart  -n version_transformations -N transformation_versioning_job -R condorpool  -s pegasus-data/metadata_transformations.yaml=pegasus-data/metadata_transformations.yaml -L federated-learning-example-tracker -T 2024-02-20T14:02:07+00:00 ./version_transformations -files init_env local_clustering global_clustering -names init_env local_clustering global_clustering -path /home/poseidon/workflows/FL-workflow/federated-learning-fedstack-PM/bin/init_env.py /home/poseidon/workflows/FL-workflow/federated-learning-fedstack-PM/bin/localClustering.py /home/poseidon/workflows/FL-workflow/federated-learning-fedstack-PM/bin/globalClustering.py --use-git -token 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 -owner swarmourr -repo autopush -branch main
set -e
printf "\n################[Container] Exiting pegasus lite container script ################\n"  1>&2
EOF2


chmod +x version_transformations_transformation_versioning_job-cont.sh
