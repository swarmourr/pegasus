...
cat <<EOF > global_clustering_global_clustering-cont.sh
#!/bin/sh
...
export HOME
EOF
container_env /srv >> global_clustering_global_clustering-cont.sh
cat <<EOF2 >> global_clustering_global_clustering-cont.sh
...
/bin/chmod +x global_clustering

set +e
job_ec=0
printf "\n#########################[Container] Launching user task #########################\n"  1>&2

pegasus-kickstart  -n global_clustering -N global_clustering -R condorpool  -L federated-learning-example-tracker -T 2024-02-20T14:02:07+00:00 ./global_clustering -f worker-1.novalocal_clusters.csv worker-2.novalocal_clusters.csv worker-3.novalocal_clusters.csv
set -e
printf "\n################[Container] Exiting pegasus lite container script ################\n"  1>&2
EOF2


chmod +x global_clustering_global_clustering-cont.sh
