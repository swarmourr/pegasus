...
cat <<EOF > data_tracker_tracker_wf_federated-learning-example-tracker-cont.sh
#!/bin/sh
...
export HOME
EOF
container_env /srv >> data_tracker_tracker_wf_federated-learning-example-tracker-cont.sh
cat <<EOF2 >> data_tracker_tracker_wf_federated-learning-example-tracker-cont.sh
...
pegasus-kickstart  -n data_tracker -N tracker_wf_federated-learning-example-tracker -R condorpool  -s pegasus-data/metadata_wf.yaml=pegasus-data/metadata_wf.yaml -L federated-learning-example-tracker -T 2024-02-20T14:02:07+00:00 ./data_tracker -files wf -data_dir pegasus-data -metadata_format yaml -o pegasus-data/metadata_wf -file_type workflow -pfn /home/poseidon/pegasus-integration/pegasus/packages/pegasus-api/test/workflow-example/workflow.yml -gdrive -remote_id 1KLUNrJNfCDI8Y5iXBmASUbtwqCpaHg3L -gcredentials cred
set -e
printf "\n################[Container] Exiting pegasus lite container script ################\n"  1>&2
EOF2


chmod +x data_tracker_tracker_wf_federated-learning-example-tracker-cont.sh
