...
class FederatedLearningWorkflow():
    ...
    wf_name = "federated-learning-example-tracker-tags"
    ...
    def create_workflow(self, clients, selection, rounds, model_path,initiation, main_round):
        ...
        worker_data= [File(f"{self.worker[0]}_data.csv"),File(f"{self.worker[1]}_data.csv"),File(f"{self.worker[2]}_data.csv")]
        config_file= File(f"clusters.json")
        ...
        for worker in range(len(self.worker)):
            local_clustering_output = File(f"{self.worker[worker]}_clusters.csv")
            ...
            locals()[f"local_clustering_job_{self.worker[worker]}"]=(Job("local_clustering",_id=f"local_clustering_{self.worker[worker]}", node_label=f"local_clustering_{self.worker[worker]}")
                                    ...
                                    .add_inputs(worker_data[worker],ref_dist)
        ...
        global_clustering_output=File(f"global_clustering_results.csv")
